...
export const switchCardFavorite = payload => ({ type: 'SWITCH_CARD_FAVORITE', payload })
...
switch (action.type) {
  ...
  case 'SWITCH_CARD_FAVORITE':
    return { ...state, cards: state.cards.map(card => (card.id === action.payload) ? { ...card, isFavorit: !card.isFavorite } : card) };
  default:
    return state;
  }
